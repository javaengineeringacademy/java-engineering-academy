...
void exercise1() {
    ...
    // For unique_ptr itself:
    auto uptr = std::make_unique<int[]>(10);
    uptr[0] = 42;
    assert(uptr[0] == 42);

    auto uptr2 = std::move(uptr);
    assert(uptr == nullptr);  // Original is null
    assert(uptr2[0] == 42);  // New owner has the data
...
void exercise3() {
    ...
    parent->child = child;   // Weak reference — doesn't increment ref count
    child->parent = parent;  // Strong reference — increments ref count

    std::cout << "Parent use_count: " << parent.use_count() << "\n";  // 2 (child->parent + local)
    std::cout << "Child use_count: " << child.use_count() << "\n";    // 1 (only local)

    // Safely access child via weak_ptr
