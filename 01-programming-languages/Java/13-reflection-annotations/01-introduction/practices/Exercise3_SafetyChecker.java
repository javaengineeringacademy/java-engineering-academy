...
public class Exercise3_SafetyChecker {
    ...
    public static List<String> checkInstantiationSafety(Class<?> clazz) {
        ...
        if (clazz.isInterface()) {
            issues.add("Cannot instantiate an interface");
        }

        if (Modifier.isAbstract(clazz.getModifiers())) {
            issues.add("Cannot instantiate an abstract class");
        }

        if (clazz.isPrimitive()) {
            issues.add("Cannot instantiate a primitive type");
        }

        if (clazz.isArray()) {
            issues.add("Cannot instantiate an array type directly");
        }

        if (clazz.isAnnotation()) {
            issues.add("Cannot instantiate an annotation type");
        }

        if (clazz.isEnum()) {
            issues.add("Cannot instantiate an enum type via constructor");
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("int: " + checkInstantiationSafety(int.class));
    }
}
